...
OUTPUT_DIR = "outputs/experiments/mechanism_ablation"
...
def run_one_seed(
    ...
    for sample_index in range(total_samples):
        ...
        if selected_target:
            adaptive_mab.update(
                int(candidates_np[selected_arm]),
                target_reward,
            )
        results.append({
            "seed": seed,
...
def main():
    ...
    print("\n[3/4] Running mechanism ablation...")
    print("-" * 70)
    all_results = []
    for seed in SEEDS:
        print(f"\n{'=' * 20} SEED {seed} {'=' * 20}")
    ...
    final_rows = []
    for method in seed_summary_df["method"].unique():
    ...
    print("\n" + "=" * 70)
    print("MECHANISM ABLATION SUMMARY")
    print("=" * 70)
