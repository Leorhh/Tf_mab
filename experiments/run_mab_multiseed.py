...
OUTPUT_DIR = "outputs/experiments/mab/multiseed"
DETAIL_DIR = os.path.join(OUTPUT_DIR, "details")
SUMMARY_CSV_PATH = os.path.join(
    OUTPUT_DIR,
    "mab_multiseed_summary.csv"
)
SUMMARY_JSON_PATH = os.path.join(
    OUTPUT_DIR,
    "mab_multiseed_summary.json"
)
...
def run_single_seed(
    ...
    for sample_count in range(total_samples):
        ...
        strategies = {}
        strategies["Random"] = int(
            exp_rng.integers(
                0,
                NUM_CANDIDATES
            )
        )
        strategies["Transformer-Greedy"] = (
            greedy_arm
        )
        strategies["Epsilon-Greedy"] = (
            epsilon_bandit.select_arm(
                predicted_reward,
                candidate_uncertainty,
                candidates_np,
            )
        ...
        adaptive_arm, adaptive_scores, beta = (
            adaptive_bandit.select_arm(
                predicted_reward,
                candidate_uncertainty,
                candidates_np,
            )
        ...
        transformer_rank = calculate_rank(
        ...
        if (sample_count + 1) % 1000 == 0:
            print(
                f"[Seed {seed}] "
                f"{sample_count + 1}/{total_samples}"
            )
...
def main():
    ...
    print("=" * 70)
    print("MULTI-SEED FORMAL MAB EXPERIMENT")
    print("=" * 70)
    print(f"[INFO] Seeds: {SEEDS}")
    print(f"[INFO] Samples per seed: {MAX_SAMPLES}")
    ...
    for seed in SEEDS:
        ...
        print(
            f"[INFO] Saved: {detail_path}"
        )
        seed_summary = calculate_seed_summary(
            seed_results
        )
        all_seed_summaries.append(
            seed_summary
        )
    seed_summary_df = pd.concat(
        all_seed_summaries,
        ignore_index=True,
    )
    seed_summary_path = os.path.join(
        OUTPUT_DIR,
        "mab_seed_summary.csv"
    )
    seed_summary_df.to_csv(
        seed_summary_path,
        index=False,
    )
    final_summary_df = calculate_final_summary(
        seed_summary_df
    )
    ...
    experiment_info = {
        ...
        "seed_summary_csv": seed_summary_path,
    }
    with open(
        SUMMARY_JSON_PATH,
        "w"
